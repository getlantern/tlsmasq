...
// RunTlsmasqProtocolWithInjectedClientHello uses 'seed' to build a customized
// tls.Client that would send 'clientHelloHandshake' as the ClientHello in lieu
// of whatever it should regularly send.
// It then creates a tlsmasq server and a test origin and runs a test that's
// very similar to tlsmasq.TestListenAndDial:
//
// - Handshake the parties
// - Send a clientMsg from client to server
// - Assert proper message is received and send a serverMsg
// - Assert serverMsg was received
// - Assert no errors occurred
func RunTlsmasqProtocolWithInjectedClientHello(seed int64, clientHelloHandshake []byte) error {
	var (
		secret               [52]byte
