...
func TestGenerateClientHellos(t *testing.T) {
	...
	for _, aCase := range []struct {
		...
		fileid, err := fuzzutil.GetSecureRandomIdentifier()
		...
		for i := 0; i < 5; i++ {
			...
			seed, err := fuzzutil.GetSecureRandomByteSlice(fuzzutil.CONFIG_SIZE)
			require.NoError(t, err)

			nonce, key, encryptedConfig, err := fuzzutil.EncryptFuzzConfig(seed)
			require.NoError(t, err)

			// XXX 0 here doesn't mean anything. The seed is implanted in
			// mathRand.Seed()
			aCase.tlsConfig.Rand = fuzzutil.MathRandReader(0)
			...
			fuzzInput := fuzzutil.PackFuzzInput(nonce, key, encryptedConfig, clientHelloData)
...
// fuzzutil.RunTlsmasqProtocolWithInjectedClientHello(), but does it without any mutations. Helpful for debugging
// specific cases
func TestFuzz(t *testing.T) {
	...
	for _, f := range inputFiles {
		...
		require.NoError(t, err)
		seedAsBytes, clientHelloHandshake, err := fuzzutil.DecryptAndUnpackFuzzInput(data)
		require.NoError(t, err)
		seed := int64(binary.LittleEndian.Uint64(seedAsBytes))
		// log.Println(len(clientHelloHandshake))
		// log.Println(seed)
		require.NoError(t, RunTlsmasqProtocolWithInjectedClientHello(seed, clientHelloHandshake))
	}
}
