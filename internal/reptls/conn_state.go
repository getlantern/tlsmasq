...
// ConnState is a construct of our own used to hold relevant information usually held internally on
// the tls.Conn type.
...
// NewConnState creates a connection state based on the input version and cipher suite. The suite
// should be represented in https://golang.org/pkg/crypto/tls/#pkg-constants.
//
...
func NewConnState(version, cipherSuite uint16, seq [8]byte) (*ConnState, error) {
...
func (cs ConnState) explicitNonceLen(_cipher interface{}) int {
	if _cipher == nil {
		return 0
	}
...
func (cs ConnState) maxPayloadSizeForWrite(_cipher interface{}, mac macFunction) int {
...
func (cs *ConnState) encrypt(record, payload []byte, _cipher interface{}, macFn macFunction, rand io.Reader) ([]byte, error) {
	if _cipher == nil {
...
func (cs *ConnState) decrypt(record []byte, _cipher interface{}, mac macFunction) ([]byte, recordType, error) {
	var plaintext []byte
